  ...
  try {
    ...
    return c.json({
      ...
      error: {
        ...
        message: error.message || 'An unexpected error occurred while creating the campaign'
      }
    }, 500);
  }
});
 ...
 */
app.post('/campaigns/:workflowId/actions', zValidator('json', workflowActionSchema), async (c) => {
  try {
    const workflowId = c.req.param('workflowId');
