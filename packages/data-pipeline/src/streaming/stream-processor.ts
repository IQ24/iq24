...
export class StreamProcessorImpl implements IStreamProcessor {
  ...
  private setupEventHandlers(): void {
    this.eventEmitter.on(
      "message:processed",
      this.handleMessageProcessed.bind(this),
    );
    this.eventEmitter.on("stream:error", this.handleStreamError.bind(this));
    this.eventEmitter.on(
      "checkpoint:created",
      this.handleCheckpointCreated.bind(this),
    );
        ...
        topics: [
          {
            ...
            configEntries: [
              {
                name: "retention.ms",
                value: (config.retention_hours * 3600 * 1000).toString(),
              },
              {
                name: "cleanup.policy",
                value: config.cleanup_policy || "delete",
              },
            ],
          },
        ],
      ...
      const streamProducer: StreamProducer = {
        send: async (message: StreamMessage) => {
          ...
          try {
            ...
            metrics.avg_latency =
              (metrics.avg_latency + (Date.now() - startTime)) / 2;
        ...
        sendBatch: async (messages: StreamMessage[]) => {
          ...
          try {
            ...
            this.eventEmitter.emit("batch:sent", {
              streamName,
  ...
  async createConsumer(
    ...
    try {
      ...
      await consumer.subscribe({
        topic: streamName,
        fromBeginning: config.auto_offset_reset === "earliest",
      });
  ...
  async getMetrics(streamName?: string): Promise<StreamMetrics> {
    ...
    return {
      stream_name: "all",
      messages_produced: allMetrics.reduce(
        (sum, m) => sum + m.messages_produced,
        0,
      ),
      messages_consumed: allMetrics.reduce(
        (sum, m) => sum + m.messages_consumed,
        0,
      ),
      processing_rate:
        allMetrics.reduce((sum, m) => sum + m.processing_rate, 0) /
        allMetrics.length,
      error_rate: allMetrics.reduce((sum, m) => sum + m.error_rate, 0),
      avg_latency:
        allMetrics.reduce((sum, m) => sum + m.avg_latency, 0) /
        allMetrics.length,
      last_processed: new Date(
        Math.max(...allMetrics.map((m) => m.last_processed.getTime())),
      ),
  ...
  async restoreFromCheckpoint(
    ...
    try {
      const latestCheckpointKey = await this.redis.get(
        `checkpoint:${streamName}:latest`,
      );
  ...
  private async handleMessageProcessed(event: {
    streamName: string;
  ...
  private async handleStreamError(event: {
    streamName: string;
    error: Error;
    payload: any;
  }): Promise<void> {
  ...
  private async handleCheckpointCreated(event: {
    streamName: string;
