...
export class LineageTrackerImpl implements ILineageTracker {
  ...
  private setupEventHandlers(): void {
    this.eventEmitter.on(
      "asset:registered",
      this.handleAssetRegistered.bind(this),
    );
    this.eventEmitter.on(
      "transformation:tracked",
      this.handleTransformationTracked.bind(this),
    );
  ...
  async registerAsset(asset: DataAsset): Promise<string> {
    ...
    try {
      ...
      this.logger.error("Failed to register asset", {
        error,
        asset: asset.name,
      });
      throw error;
  ...
  async trackTransformation(transformation: Transformation): Promise<void> {
    ...
    try {
      ...
      this.eventEmitter.emit("transformation:tracked", {
        transformation,
        transformationId,
      });
    } catch (error) {
      this.logger.error("Failed to track transformation", {
        error,
  ...
  async createLineageEdge(
    ...
    try {
      ...
      this.logger.error("Failed to create lineage edge", {
        error,
        source,
        target,
      });
  ...
  async analyzeImpact(
    ...
    try {
      ...
      const criticalAssets = impactedAssets
        .filter(
          (asset) => asset.impact_score > this.config.analysis.impact_threshold,
        )
      ...
      const analysis: ImpactAnalysis = {
        ...
        estimated_impact_score: impactedAssets.reduce(
          (sum, asset) => sum + asset.impact_score,
          0,
        ),
      ...
      this.logger.error("Failed to analyze impact", {
        error,
        assetId,
        changeType,
      });
  ...
  async trackSchemaChange(
    ...
    try {
      ...
      this.eventEmitter.emit("schema:changed", {
        assetId,
        oldSchema,
        newSchema,
        changes,
  ...
  private generateImpactRecommendations(
    ...
    const criticalCount = impactedAssets.filter(
      (a) => a.risk_level === "critical",
    ).length;
    const highCount = impactedAssets.filter(
      (a) => a.risk_level === "high",
    ).length;

    if (criticalCount > 0) {
      recommendations.push(
        `${criticalCount} critical assets will be impacted. Consider phased rollout.`,
      );
    }

    if (highCount > 0) {
      recommendations.push(
        `${highCount} high-risk assets require careful monitoring during deployment.`,
      );
    }

    if (changeType === "schema_breaking") {
      recommendations.push(
        "Schema breaking change detected. Ensure backward compatibility or coordinate with downstream teams.",
      );
    }
  ...
  private compareSchemas(oldSchema: any, newSchema: any): SchemaChange[] {
    ...
    for (const field of oldFields) {
      if (
        ...
        JSON.stringify(oldSchema[field]) !== JSON.stringify(newSchema[field])
      ) {
        changes.push({
          type: "modified",
          field,
  ...
  private async handleAssetRegistered(event: {
    asset: DataAsset;
  }): Promise<void> {
    this.logger.debug("Asset registered event handled", {
      assetId: event.asset.id,
    });
  }

  private async handleTransformationTracked(event: {
    transformation: Transformation;
    transformationId: string;
  }): Promise<void> {
    this.logger.debug("Transformation tracked event handled", {
      transformationId: event.transformationId,
    });
  }

  private async handleSchemaChanged(event: {
    assetId: string;
    oldSchema: any;
    newSchema: any;
    changes: SchemaChange[];
  ...
  async listAssets(filter?: Partial<DataAsset>): Promise<DataAsset[]> {
    ...
    return assets.filter((asset) =>
      Object.entries(filter).every(
        ([key, value]) => asset[key as keyof DataAsset] === value,
      ),
    );
  }
  ...
  async getLineageHealth(assetId: string): Promise<HealthStatus> {
    ...
    return {
      ...
      details: {
        ...
        has_downstream: !!(await this.getDownstreamLineage(assetId)).nodes
          .length,
      },
    };
  }
