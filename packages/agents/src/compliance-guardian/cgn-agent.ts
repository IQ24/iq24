...
export class ComplianceGuardianAgent extends BaseAgent {
  ...
  constructor() {
    const metadata: AgentMetadata = {
      ...
      version: "1.0.0",
      description:
        "Proactive compliance monitoring and enforcement for global data protection and marketing regulations",
      capabilities: [
  ...
  private async screenContent(
    ...
    complianceCheck.overallStatus = this.determineOverallStatus(
    ...
    if (complianceCheck.violations.length > 0) {
      complianceCheck.recommendations =
        await this.generateComplianceRecommendations(
    ...
    const blockingViolations = complianceCheck.violations.filter(
    ...
    return {
      ...
      data: {
        ...
        complianceCheck,
        blockingViolations,
        requiresReview: complianceCheck.violations.some(
          (v) => v.severity === "high",
        ),
      },
      metadata: {
  ...
  private async checkCompliance(
    ...
    if (payload.scope === "campaign" || payload.scope === "campaign") {
      const campaignCheck = await this.checkCampaignCompliance(
        payload.campaignId,
      );
      ...
      for (const content of payload.content) {
        ...
        if (contentCheck.data?.complianceCheck?.violations) {
          complianceResults.violations.push(
            ...contentCheck.data.complianceCheck.violations,
          );
        }
      }
    }
  ...
  private async generateComplianceReport(
    ...
    const report: ComplianceReport = {
      ...
      violations: await this.getViolationsInRange(
        payload.timeRange,
        payload.scope,
      ),
      auditTrail:
        payload.reportType === "audit"
    ...
    return [
      {
        ...
        description:
          "Require express consent for commercial electronic messages",
        regulationType: "CASL",
        severity: "critical",
        applicableChannels: ["email", "sms"],
        requiredElements: [
  ...
  private isCaliforniaLocation(location: string): boolean {
    return (
      location.toLowerCase().includes("ca") ||
