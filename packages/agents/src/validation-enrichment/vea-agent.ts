...
import { AgentConfig, TaskContext, WorkflowTask, DataSourceConfig, EnrichmentResult } from '@iq24/ai-core/types';
...
export class ValidationEnrichmentAgent extends BaseAgent {
   ...
   */
  private async validateContacts(candidates: any[], context: TaskContext): Promise<ValidationResult[]> {
   ...
   */
  private async enrichProfiles(leads: ValidationResult[], context: TaskContext): Promise<ValidationResult[]> {
    ...
    for (const lead of leads) {
      ...
      try {
        ...
        this.logger.warn('Profile enrichment failed', { leadId: lead.leadId, error });
   ...
   */
  private async performMultiSourceEnrichment(lead: ValidationResult, context: TaskContext): Promise<{
      ...
      try {
        ...
        if (providerData && Object.keys(providerData).length > 0) {
          ...
          for (const [key, value] of Object.entries(providerData)) {
            if (!enrichmentData[key] || this.shouldOverrideData(enrichmentData[key], value, providerName)) {
        ...
        this.logger.warn(`Enrichment provider ${providerName} failed`, { error, leadId: lead.leadId });
   ...
   */
  private async validateEmail(email: string, context: TaskContext): Promise<{
    isValid: boolean;
    confidence: number;
    technique: string;
  }> {
   ...
   */
  private async validatePhone(phone: string, context: TaskContext): Promise<{
    isValid: boolean;
    confidence: number;
    formatted?: string;
  }> {
   ...
   */
  private async deduplicateData(dataset: ValidationResult[], context: TaskContext): Promise<{
   ...
   */
  private async crossReferenceData(sources: DataSourceConfig[], context: TaskContext): Promise<{
    consistency: number;
  ...
  // Helper methods
  private calculateValidationScore(emailValidation: any, phoneValidation: any): number {
    const emailWeight = 0.7;
    const phoneWeight = 0.3;
    
    return (emailValidation.confidence * emailWeight) + (phoneValidation.confidence * phoneWeight);
  }
  ...
  private getValidationReasons(emailValidation: any, phoneValidation: any): string[] {
  ...
  private async callEnrichmentProvider(provider: string, lead: ValidationResult, context: TaskContext): Promise<Record<string, any> | null> {
  ...
  private async callApolloEnrichment(lead: ValidationResult): Promise<Record<string, any> | null> {
  ...
  private async callHunterEnrichment(lead: ValidationResult): Promise<Record<string, any> | null> {
  ...
  private async callClearbitEnrichment(lead: ValidationResult): Promise<Record<string, any> | null> {
  ...
  private shouldOverrideData(existing: any, newValue: any, provider: string): boolean {
    ...
    return (providerReliability[provider as keyof typeof providerReliability] || 0.5) > 0.8;
  }
