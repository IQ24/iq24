  ...
  private async runSuite(suite: TestSuite): Promise<TestResult[]> {
    ...
    for (const test of suite.tests) {
      ...
      try {
        ...
        console.log(
          `    ${result.success ? "✅" : "❌"} ${test.name} (${duration}ms)`,
        );
      ...
      tests: [
        {
          ...
          description:
            "Verify all 7 AI agents can be instantiated and initialized",
          execute: async () => {
            const agents = [
        ...
        },
        {
          ...
          execute: async () => {
            ...
            const newAgent = await this.createMockAgent(
              "ProspectDiscoveryAgent",
            );
            const restoredState = await newAgent.loadState();

            const stateMatches =
              JSON.stringify(testState) === JSON.stringify(restoredState);
      ...
      tests: [
        {
          ...
          execute: async () => {
            ...
            try {
              // Start workflow
              const workflowId =
                await this.workflow.startCampaign(mockCampaignData);

              // Monitor workflow progress
              let attempts = 0;
              let workflowCompleted = false;

              while (attempts < 30 && !workflowCompleted) {
                // 30 second timeout
                const status =
                  await this.workflow.getWorkflowStatus(workflowId);
        ...
        },
        {
          ...
          description:
            "Test multiple agents can work on different campaigns simultaneously",
          execute: async () => {
            const campaigns = [
      ...
      tests: [
        {
          ...
          execute: async () => {
            try {
              ...
              return {
                success:
                  retrievedLead?.email === testLead.email &&
      ...
      tests: [
        {
          ...
          execute: async () => {
            ...
            return {
              success:
                unhealthyStatus.status !== "healthy" &&
        ...
        },
        {
          ...
          description:
            "Test system continues operating with reduced functionality",
          execute: async () => {
      ...
      tests: [
        {
          ...
          description:
            "Test system can handle multiple campaigns simultaneously",
          execute: async () => {
            const concurrentCampaigns = 5;
            const campaigns = Array.from(
              { length: concurrentCampaigns },
              (_, i) => ({
        ...
        },
        {
          ...
          execute: async () => {
            ...
            for (let i = 0; i < 10; i++) {
              const agent = await this.createMockAgent(
                "ProspectDiscoveryAgent",
              );
      ...
      tests: [
        {
          ...
          execute: async () => {
            ...
            return {
              success:
                anonymizedLead?.email !== testLead.email &&
                deletedLead === null,
              metadata: {
