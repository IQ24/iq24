...
export class MetricsCollector extends EventEmitter {
  ...
  private async collectExternalServiceMetrics(timestamp: number): Promise<void> {
  ...
  private getRandomIncreasing(metricName: string, maxIncrease: number = 10): number {
    ...
    this.addRule({
      ...
      condition: (metrics) => {
        ...
        return services.some(service => {
          const metric = metrics.get(`external_services.${service}.availability`);
...
export class SystemMonitor extends EventEmitter {
  ...
  getSystemHealth(): SystemHealthStatus {
    ...
    const metrics: PerformanceMetrics = {
      ...
      memory: {
        used: this.getLatestMetricValue(allMetrics, 'system.memory.heap_used') || 0,
        total: this.getLatestMetricValue(allMetrics, 'system.memory.heap_total') || 0,
        percentage: 0,
        heapUsed: this.getLatestMetricValue(allMetrics, 'system.memory.heap_used') || 0,
        heapTotal: this.getLatestMetricValue(allMetrics, 'system.memory.heap_total') || 0
      },
      network: {
        connections: 0,
        bytesIn: 0,
      ...
      agents: {
        ...
        processing: this.getLatestMetricValue(allMetrics, 'agents.processing') || 0,
        idle: 0,
        failed: 0
      },
      workflows: {
        running: this.getLatestMetricValue(allMetrics, 'workflows.running') || 0,
        completed: this.getLatestMetricValue(allMetrics, 'workflows.completed_today') || 0,
        failed: 0,
        avgDuration: 0
      },
      database: {
        connections: this.getLatestMetricValue(allMetrics, 'database.active_connections') || 0,
        queries: this.getLatestMetricValue(allMetrics, 'database.query_count') || 0,
        avgResponseTime: 0
      },
      external_services: {}
    };
    ...
    for (const service of services) {
      metrics.external_services[service] = {
        available: this.getLatestMetricValue(allMetrics, `external_services.${service}.availability`) === 1,
        responseTime: this.getLatestMetricValue(allMetrics, `external_services.${service}.response_time`) || 0,
        errorRate: 0
      };
    }
  ...
  private getLatestMetricValue(allMetrics: Record<string, MetricValue[]>, metricName: string): number | null {
