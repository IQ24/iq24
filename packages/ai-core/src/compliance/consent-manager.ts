...
export interface ConsentStorageAdapter {
  ...
  getConsent(contactId: string, type: ConsentType): Promise<ConsentRecord | null>;
...
export interface NotificationService {
  sendConsentConfirmation(contact: ContactInfo, consent: ConsentRecord): Promise<void>;
  sendWithdrawalConfirmation(contact: ContactInfo, consent: ConsentRecord): Promise<void>;
  sendDataRightsResponse(request: DataRightsRequest, response: string): Promise<void>;
  sendExpirationNotice(contact: ContactInfo, consent: ConsentRecord): Promise<void>;
}
...
export class ConsentManager extends EventEmitter {
  ...
  async recordConsent(
    ...
    try {
      ...
      await this.validateConsentRequirements(contactInfo, consentType, framework);
      ...
      const consent: ConsentRecord = {
        ...
        type: consentType,
        purposes,
        status: options.doubleOptIn ? ConsentStatus.PENDING_CONFIRMATION : ConsentStatus.GRANTED,
        framework,
        legalBasis: options.legalBasis || this.determineLegalBasis(consentType, framework),
        grantedAt: new Date(),
        ...
        confirmationToken: options.doubleOptIn ? this.generateConfirmationToken() : undefined,
  ...
  async confirmConsent(confirmationToken: string): Promise<ConsentRecord> {
    try {
      ...
      const consent = consents.find(c => c.confirmationToken === confirmationToken);
      if (!consent) {
      ...
      confirmationExpiry.setHours(confirmationExpiry.getHours() + this.config.confirmationExpiryHours);
  ...
  async withdrawConsent(
    ...
    try {
      ...
      if (contactInfo) {
        await this.notifications.sendWithdrawalConfirmation(contactInfo, updatedConsent);
      }
    ...
    try {
      ...
      switch (request.type) {
        ...
        case DataRightsType.OBJECTION:
          ...
          break;
        default:
          throw new Error(`Unsupported data rights request type: ${request.type}`);
      }
   ...
   */
  async getConsentMetrics(timeRange?: { from: Date; to: Date }): Promise<ConsentMetrics> {
  ...
  private determineLegalBasis(consentType: ConsentType, framework: RegulatoryFramework): LegalBasis {
  ...
  private calculateExpiryDate(framework: RegulatoryFramework): Date | undefined {
  ...
  private setupAutomaticProcessing(): void {
    ...
    setInterval(async () => {
      try {
        ...
        this.logger.error('Automatic consent processing failed', { error: error.message });
      }
  ...
  private async processAccessRequest(request: DataRightsRequest): Promise<string> {
  ...
  private async processDeletionRequest(request: DataRightsRequest): Promise<string> {
  ...
  private async processRectificationRequest(request: DataRightsRequest): Promise<string> {
  ...
  private async processPortabilityRequest(request: DataRightsRequest): Promise<string> {
    // Implementation for data portability (GDPR Article 20)
    return await this.processAccessRequest(request);
  }

  private async processRestrictionRequest(request: DataRightsRequest): Promise<string> {
  ...
  private async processObjectionRequest(request: DataRightsRequest): Promise<string> {
    // Implementation for right to object (GDPR Article 21)
    return await this.withdrawConsent(request.contactId, ConsentType.MARKETING, 'User objection');
  }
}
...
class ConsentMetricsCollector {
  ...
  private timeSeriesData: Array<{ timestamp: Date; event: string; framework: string }> = [];
  ...
  getMetrics(timeRange?: { from: Date; to: Date }): ConsentMetrics {
    ...
    if (timeRange) {
      data = data.filter(d => d.timestamp >= timeRange.from && d.timestamp <= timeRange.to);
    }
  ...
  private groupByFramework(data: Array<{ event: string; framework: string }>): Record<string, number> {
