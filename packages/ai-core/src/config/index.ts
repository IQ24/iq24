  ...
  return {
    ...
    aiServices: {
      ...
      anthropic: {
        ...
        models: {
          completion:
            process.env.ANTHROPIC_COMPLETION_MODEL ||
      ...
      mistral: {
        ...
        models: {
          completion:
            process.env.MISTRAL_COMPLETION_MODEL || "mistral-large-latest",
    ...
    dataSources: {
      ...
      clearbit: {
        ...
        baseUrl:
          process.env.CLEARBIT_BASE_URL || "https://person.clearbit.com/v2",
      },
    },
    workflow: {
      temporal: {
    ...
    agents: {
      pda: {
        maxConcurrentDiscoveries: parseInt(
          process.env.PDA_MAX_CONCURRENT || "5",
        ),
        discoveryTimeoutMs: parseInt(process.env.PDA_TIMEOUT_MS || "300000"),
        retryAttempts: parseInt(process.env.PDA_RETRY_ATTEMPTS || "3"),
        dataSources: (
          process.env.PDA_DATA_SOURCES || "apollo,hunter,linkedin"
        ).split(","),
      },
      vea: {
        maxConcurrentValidations: parseInt(
          process.env.VEA_MAX_CONCURRENT || "10",
        ),
        validationTimeoutMs: parseInt(process.env.VEA_TIMEOUT_MS || "60000"),
        enrichmentSources: (
          process.env.VEA_ENRICHMENT_SOURCES || "apollo,clearbit,hunter"
        ).split(","),
        deduplicationThreshold: parseFloat(
          process.env.VEA_DEDUP_THRESHOLD || "0.85",
        ),
      },
      opa: {
        maxConcurrentPersonalizations: parseInt(
          process.env.OPA_MAX_CONCURRENT || "8",
        ),
        personalizationTimeoutMs: parseInt(
          process.env.OPA_TIMEOUT_MS || "120000",
        ),
      ...
      cea: {
        maxConcurrentExecutions: parseInt(
          process.env.CEA_MAX_CONCURRENT || "20",
        ),
      ...
      cgn: {
        complianceRules: (
          process.env.CGN_COMPLIANCE_RULES || "gdpr,ccpa,canspam"
        ).split(","),
      ...
      crm: {
        learningIntervalMs: parseInt(
          process.env.CRM_LEARNING_INTERVAL_MS || "86400000",
        ), // 24 hours
        knowledgeTransferThreshold: parseFloat(
          process.env.CRM_KNOWLEDGE_THRESHOLD || "0.8",
        ),
    ...
    compliance: {
      ...
      canSpam: {
        ...
        unsubscribeRequired:
          process.env.CAN_SPAM_UNSUBSCRIBE_REQUIRED !== "false",
        identificationRequired:
          process.env.CAN_SPAM_IDENTIFICATION_REQUIRED !== "false",
      },
    },
    monitoring: {
      logging: {
      ...
      alerts: {
        ...
        thresholds: {
          ...
          queueLength: parseInt(
            process.env.ALERT_QUEUE_LENGTH_THRESHOLD || "100",
          ),
        },
      },
    },
...
export function validateConfig(config: AIConfig): void {
  ...
  if (
    ...
    errors.push(
      "SUPABASE_SERVICE_KEY is required for non-development environments",
    );
  }

  // AI Services validation
  if (
    !config.aiServices.openai.apiKey &&
    config.environment !== "development"
  ) {
