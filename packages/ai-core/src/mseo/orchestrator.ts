...
export class MSEOOrchestrator extends EventEmitter {
  ...
  async generateContent(
    ...
    try {
      ...
      this.emit(
        "queueUpdated",
        this.contentQueue.length,
        this.getQueuePriorityDistribution(),
      );

      // Return promise that resolves when processing is complete
      return new Promise((resolve, reject) => {
        const checkResult = () => {
          const queueIndex = this.contentQueue.findIndex(
      ...
      this.logger.error(
        `Content generation failed for request ${requestId}:`,
        error,
      );
  ...
  private async processMultimodalRequest(
    ...
    results.forEach((result) => {
      ...
      if (
        ...
        !providersUsed.includes(result.metadata.providerUsed)
      ) {
    ...
    const defaultTemplates: MSEOTemplate[] = [
      {
        ...
        variables: {
          script:
            "Hi {{prospect.firstName}}, this is {{user.name}} from {{company.name}}. I noticed {{prospect.company}} is {{prospect.industry}} and thought you might be interested in how we helped {{similarCompany}} achieve {{achievement}}.",
        },
      ...
      },
      {
        ...
        variables: {
          prompt:
            "Professional product showcase for {{product.name}}, highlighting {{product.keyFeature}}, styled for {{prospect.industry}} industry, clean modern design, corporate aesthetic",
        },
        personalizationRules: {
          industrySpecific: true,
      ...
      },
      {
        ...
        variables: {
          script:
            "Hi {{prospect.firstName}}, let me show you how {{product.name}} can specifically help {{prospect.company}} with {{prospect.painPoint}}.",
          visualPrompt:
            "Screen recording of software demo, professional presenter, {{prospect.company}} logo integration, {{prospect.industry}} use case focus",
        },
        personalizationRules: {
          industrySpecific: true,
          roleSpecific: true,
      ...
      try {
        ...
        const promises = itemsToProcess.map(async (item) => {
          try {
            ...
            this.logger.error(
              `Queue item processing failed: ${item.id}`,
              error,
            );
  ...
  private checkPerformanceAlerts(): void {
    ...
    if (this.analytics.performanceMetrics.latency > thresholds.maxLatency) {
      this.emit(
        "performanceAlert",
        "latency",
        this.analytics.performanceMetrics.latency,
    ...
    // Check throughput
    if (
      this.analytics.performanceMetrics.throughput < thresholds.minThroughput
    ) {
      this.emit(
        "performanceAlert",
        "throughput",
  ...
  private checkCostAlerts(): void {
    ...
    if (budgets.daily && this.analytics.costMetrics.totalCost > budgets.daily) {
      this.emit(
        "costAlert",
        "overall",
        this.analytics.costMetrics.totalCost,
        budgets.daily,
      );
    }
    ...
    Object.entries(budgets.perProvider || {}).forEach(([provider, budget]) => {
      const providerCost =
        this.analytics.costMetrics.providerCosts[provider] || 0;
  ...
  private updateAnalytics(
    ...
    if (result.metadata.providerUsed) {
      this.analytics.costMetrics.providerCosts[result.metadata.providerUsed] =
        (this.analytics.costMetrics.providerCosts[
          result.metadata.providerUsed
        ] || 0) + result.metadata.cost;
    }

    // Update quality metrics
    const qualitySum =
      this.analytics.qualityMetrics.averageQualityScore *
        (this.analytics.totalRequests - 1) +
      result.metadata.qualityScore;
    this.analytics.qualityMetrics.averageQualityScore =
      qualitySum / this.analytics.totalRequests;
  ...
  async getQueueStatus(): Promise<{
    ...
    return {
      ...
      estimatedProcessingTime:
        this.contentQueue.length *
        (this.analytics.averageGenerationTime || 5000),
    };
  }
