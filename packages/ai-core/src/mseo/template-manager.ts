...
export class MSEOTemplateManager {
  ...
  private initializeDefaultTemplates(): void {
    const defaultTemplates: MSEOTemplate[] = [
      // Voice Templates
      {
        ...
        variables: [
          { name: 'firstName', type: 'string', required: true, description: 'Prospect first name' },
          { name: 'senderName', type: 'string', required: true, description: 'Sender full name' },
          { name: 'companyName', type: 'string', required: true, description: 'Sender company name' },
          { name: 'prospectCompany', type: 'string', required: true, description: 'Prospect company name' },
          { name: 'recentActivity', type: 'string', required: false, description: 'Recent company activity or news' },
          { name: 'industry', type: 'string', required: true, description: 'Target industry' },
          { name: 'similarCompany', type: 'string', required: false, description: 'Similar successful client' },
          { name: 'benefit', type: 'string', required: true, description: 'Key benefit or result achieved' }
        ],
        personalizationRules: [
          {
            condition: 'if industry == "fintech"',
            action: 'replace "achieve" with "secure compliance while achieving"'
          },
          {
      ...
      // Image Templates
      {
        ...
        variables: [
          { name: 'firstName', type: 'string', required: true, description: 'Prospect first name' },
          { name: 'industry', type: 'string', required: true, description: 'Target industry' },
          { name: 'companyLogo', type: 'url', required: false, description: 'Company logo URL' }
        ],
        personalizationRules: [
          {
      ...
      // Video Templates
      {
        ...
        variables: [
          { name: 'firstName', type: 'string', required: true, description: 'Prospect first name' },
          { name: 'productName', type: 'string', required: true, description: 'Product or service name' },
          { name: 'painPoint', type: 'string', required: true, description: 'Specific pain point to address' },
          { name: 'prospectCompany', type: 'string', required: true, description: 'Prospect company name' },
          { name: 'industry', type: 'string', required: true, description: 'Target industry' }
        ],
        personalizationRules: [
  ...
  addTemplate(template: MSEOTemplate): void {
    ...
    // Index by industry
    const industryTemplates = this.templatesByIndustry.get(template.industry) || [];
   ...
   */
  async findBestTemplate(request: MSEOGenerationRequest, personalizationData: MSEOPersonalizationData): Promise<MSEOTemplateMatch | null> {
    ...
    const scoredCandidates = candidates.map(template => {
      ...
      const industryMatch = template.industry === criteria.industry ? 1 : 
                           template.industry === 'generic' ? 0.5 : 0;
      
      // Tone match bonus
      const toneMatch = template.tone === criteria.tone ? 0.5 : 0;
      
      // Tag relevance score
      const tagScore = this.calculateTagRelevance(template.tags, criteria.tags || []);
      
      const totalScore = baseScore + conversionScore + usageScore + industryMatch + toneMatch + tagScore;
    ...
    return {
      ...
      personalizationSuggestions: this.generatePersonalizationSuggestions(bestMatch.template, personalizationData),
  ...
  searchTemplates(criteria: MSEOTemplateCriteria): MSEOTemplate[] {
    ...
    if (criteria.industry) {
      templates = templates.filter(t => 
        t.industry === criteria.industry || t.industry === 'generic'
      );
    }
    ...
    if (criteria.minQualityScore) {
      templates = templates.filter(t => (t.qualityScore || 0) >= criteria.minQualityScore);
    }
   ...
   */
  async personalizeTemplate(template: MSEOTemplate, personalizationData: MSEOPersonalizationData): Promise<string> {
    let personalizedContent = template.template;

    // Replace template variables
    template.variables.forEach(variable => {
      const value = personalizationData[variable.name as keyof MSEOPersonalizationData] || '';
    ...
    if (template.personalizationRules) {
      for (const rule of template.personalizationRules) {
        if (this.evaluateCondition(rule.condition, personalizationData)) {
          personalizedContent = this.applyRuleAction(personalizedContent, rule.action, personalizationData);
        }
      }
    }
    ...
    if (this.config.enableAIEnhancements) {
      personalizedContent = await this.enhanceWithAI(personalizedContent, personalizationData, template);
    }
   ...
   */
  private generatePersonalizationSuggestions(template: MSEOTemplate, data: MSEOPersonalizationData): string[] {
    const suggestions: string[] = [];

    // Missing required variables
    const missingRequired = template.variables
      .filter(v => v.required && !data[v.name as keyof MSEOPersonalizationData])
    ...
    if (data.industry) {
      switch (data.industry) {
        case 'fintech':
          suggestions.push('Consider emphasizing security and compliance benefits');
          break;
        case 'healthcare':
          suggestions.push('Highlight HIPAA compliance and patient privacy features');
          break;
        case 'ecommerce':
    ...
    if (data.companySize) {
      if (data.companySize < 50) {
        suggestions.push('Emphasize ease of use and quick implementation for small teams');
   ...
   */
  recordTemplateUsage(templateId: string, success: boolean, generationTime: number, cost: number): void {
    ...
    // Update averages
    const totalGenerations = analytics.successfulGenerations + analytics.failedGenerations;
    analytics.averageGenerationTime = (analytics.averageGenerationTime * (totalGenerations - 1) + generationTime) / totalGenerations;
    analytics.costPerGeneration = (analytics.costPerGeneration * (totalGenerations - 1) + cost) / totalGenerations;
   ...
   */
  getTemplateAnalytics(templateId?: string): MSEOTemplateAnalytics | MSEOTemplateAnalytics[] {
    if (templateId) {
      return this.templateAnalytics.get(templateId) || {} as MSEOTemplateAnalytics;
    }
   ...
   */
  updateTemplatePerformance(templateId: string, conversionRate: number, qualityScore: number): void {
    const template = this.templates.get(templateId);
    const analytics = this.templateAnalytics.get(templateId);
   ...
   */
  getTopPerformingTemplates(type?: MSEOTemplateType, limit: number = 10): MSEOTemplate[] {
    let templates = Array.from(this.templates.values());
    ...
    return templates
      .filter(t => t.isActive)
      .sort((a, b) => {
        const scoreA = (a.conversionRate || 0) * 0.6 + (a.qualityScore || 0) * 0.4;
        const scoreB = (b.conversionRate || 0) * 0.6 + (b.qualityScore || 0) * 0.4;
   ...
   */
  validateTemplate(template: MSEOTemplate): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    referencedVariables.forEach(varName => {
      if (!template.variables.find(v => v.name === varName)) {
        errors.push(`Variable '${varName}' is used in template but not defined`);
      }
    });
  ...
  private calculateTagRelevance(templateTags: string[], requestTags: string[]): number {
    if (requestTags.length === 0) return 0;
    
    const matchingTags = templateTags.filter(tag => requestTags.includes(tag));
    return matchingTags.length / requestTags.length;
  }

  private generateMatchReasons(template: MSEOTemplate, criteria: MSEOTemplateCriteria, scores: any): string[] {
    ...
    if (template.conversionRate && template.conversionRate > 0.1) {
      reasons.push(`High conversion rate: ${(template.conversionRate * 100).toFixed(1)}%`);
    }
    
    if (template.qualityScore && template.qualityScore > 0.8) {
      reasons.push(`High quality score: ${(template.qualityScore * 100).toFixed(0)}%`);
    }
    
    return reasons;
  }

  private evaluateCondition(condition: string, data: MSEOPersonalizationData): boolean {
  ...
  private applyRuleAction(content: string, action: string, data: MSEOPersonalizationData): string {
  ...
  private async enhanceWithAI(content: string, data: MSEOPersonalizationData, template: MSEOTemplate): Promise<string> {
 ...
 */
export function createMSEOTemplateManager(config: MSEOTemplateConfig): MSEOTemplateManager {
