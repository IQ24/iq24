...
export class ImageEngine extends EventEmitter {
  ...
  async generatePersonalizedImage(
    ...
    try {
      ...
      const personalizedPrompt = this.personalizePrompt(template.prompt, variables, prospectData);
      ...
      if (params.provider === 'stability' && this.config.providers.stability.enabled) {
   ...
   */
  private async executeImageGeneration(params: ImageGenerationParams): Promise<ImageGenerationResponse> {
   ...
   */
  private async generateWithOpenAI(params: ImageGenerationParams, startTime: number): Promise<ImageGenerationResponse> {
    ...
    const totalCost = this.calculateCost('openai', params.prompt.length, data.data.length);
   ...
   */
  private async generateWithStability(params: ImageGenerationParams, startTime: number): Promise<ImageGenerationResponse> {
    ...
    const data = await response.json();
    const generationTime = Date.now() - startTime;
    const totalCost = this.calculateCost('stability', params.prompt.length, data.artifacts.length);
   ...
   */
  private async generateWithMidjourney(params: ImageGenerationParams, startTime: number): Promise<ImageGenerationResponse> {
    ...
    throw new Error('Midjourney provider not yet implemented - placeholder for future integration');
  }
  ...
  private personalizePrompt(
    ...
    if (prospectData) {
      const replacements = {
        'prospect_name': prospectData.firstName || prospectData.name || 'Professional',
    ...
    if (this.config.personalizationLevel === 'hyper') {
      personalizedPrompt += ', highly detailed, professional quality, modern style';
  ...
  private async checkRateLimit(provider: string): Promise<void> {
    ...
    const maxRequestsPerMinute = provider === 'openai' ? 5 : provider === 'stability' ? 10 : 3;
   ...
   */
  private calculateCost(provider: string, promptLength: number, imageCount: number): number {
  ...
  async downloadImage(url: string, localPath: string): Promise<string> {
    try {
      ...
      this.logger.info('Image downloaded successfully', { url, localPath, size: buffer.byteLength });
      return localPath;
    } catch (error) {
      this.logger.error('Failed to download image', { error: error.message, url });
      throw error;
    }
  }
  ...
  async shutdown(): Promise<void> {
    ...
    if (activeGenerations.length > 0) {
      this.logger.info(`Waiting for ${activeGenerations.length} active image generations to complete...`);
