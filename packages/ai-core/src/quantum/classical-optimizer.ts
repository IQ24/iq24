...
export class ClassicalOptimizer {
  ...
  async optimize(
    ...
    const algorithm = config?.algorithm as ClassicalAlgorithmType || this.config.defaultAlgorithm;
  ...
  private async geneticAlgorithm(
    ...
    for (let generation = 0; generation < params.maxGenerations; generation++) {
      // Selection
      const parents = this.tournamentSelection(population, problem, params.tournamentSize);
      ...
      population = this.evaluatePopulation(combined, problem)
      ...
      // Check convergence
      if (this.checkConvergence(fitnessHistory, this.config.convergenceThreshold)) {
        break;
      }
    }

    return this.convertToOptimizationSolution(bestSolution, problem, 'genetic_algorithm');
  }
  ...
  private async simulatedAnnealing(
    ...
    for (let iteration = 0; iteration < params.maxIterations && temperature > params.minTemperature; iteration++) {
      // Generate neighbor
      const neighbor = this.generateNeighbor(currentSolution, problem, params.neighborhoodSize);
    ...
    return this.convertToOptimizationSolution(bestSolution, problem, 'simulated_annealing');
  }
  ...
  private async particleSwarmOptimization(
    ...
    return this.convertToOptimizationSolution(globalBest, problem, 'particle_swarm');
  }
  ...
  private async hillClimbing(
    ...
    return this.convertToOptimizationSolution(currentSolution, problem, 'hill_climbing');
  }
  ...
  private async tabuSearch(
    ...
    for (let iteration = 0; iteration < maxIterations; iteration++) {
      ...
      for (const neighbor of neighbors) {
        if (!this.isTabu(neighbor, tabuList) || neighbor.fitness > bestSolution.fitness) {
    ...
    return this.convertToOptimizationSolution(bestSolution, problem, 'tabu_search');
  }
  ...
  private async differentialEvolution(
    ...
    return this.convertToOptimizationSolution(bestSolution, problem, 'differential_evolution');
  }
  ...
  private generateRandomSolution(problem: OptimizationProblem): any {
    ...
    switch (problem.type) {
      case 'campaign_optimization':
        ...
        solution.variables = {
          channelWeights: this.generateRandomWeights(campProblem.channels.length),
      ...
      case 'resource_allocation':
        ...
        solution.variables = {
          allocation: this.generateRandomAllocation(resProblem.resources, resProblem.constraints.totalBudget)
        };
        break;
      ...
      case 'prospect_prioritization':
        ...
        solution.variables = {
          ...
          weights: this.generateRandomWeights(prosProblem.scoringCriteria.length)
        };
        break;
    }
  ...
  private generateRandomWeights(count: number): number[] {
    const weights = Array(count).fill(0).map(() => Math.random());
    const sum = weights.reduce((a, b) => a + b, 0);
    return weights.map(w => w / sum);
  }

  private generateRandomAllocation(resources: any[], totalBudget: number): { [key: string]: number } {
  ...
  private evaluateSolution(solution: any, problem: OptimizationProblem): number {
    // This would typically call the problem's objective function
    // For now, return a placeholder based on solution complexity
    return Math.random() * 100;
  }

  private evaluatePopulation(population: any[], problem: OptimizationProblem): any[] {
    return population
      .map(individual => ({
        ...individual,
  ...
  private tournamentSelection(population: any[], problem: OptimizationProblem, tournamentSize: number): any[] {
    const parents = [];
  ...
  private crossover(parents: any[], crossoverRate: number, problem: OptimizationProblem): any[] {
    const offspring = [];
    
    for (let i = 0; i < parents.length - 1; i += 2) {
      if (Math.random() < crossoverRate) {
        const [child1, child2] = this.performCrossover(parents[i], parents[i + 1], problem);
  ...
  private performCrossover(parent1: any, parent2: any, problem: OptimizationProblem): [any, any] {
    ...
    switch (problem.type) {
      case 'campaign_optimization':
        ...
        for (let i = 0; i < parent1.variables.channelWeights.length; i++) {
          if (Math.random() < 0.5) {
            child1.variables.channelWeights[i] = parent2.variables.channelWeights[i];
            child2.variables.channelWeights[i] = parent1.variables.channelWeights[i];
          }
        }
        break;
    }
  ...
  private mutate(population: any[], mutationRate: number, problem: OptimizationProblem): void {
  ...
  private performMutation(individual: any, problem: OptimizationProblem): void {
    switch (problem.type) {
      case 'campaign_optimization':
        ...
        const index = Math.floor(Math.random() * individual.variables.channelWeights.length);
        individual.variables.channelWeights[index] += (Math.random() - 0.5) * 0.1;
        individual.variables.channelWeights[index] = Math.max(0, Math.min(1, individual.variables.channelWeights[index]));
        
        // Normalize weights
        const sum = individual.variables.channelWeights.reduce((a: number, b: number) => a + b, 0);
        individual.variables.channelWeights = individual.variables.channelWeights.map((w: number) => w / sum);
        break;
    }
  }

  private checkConvergence(fitnessHistory: number[], threshold: number): boolean {
  ...
  private initializeSwarm(problem: OptimizationProblem, particleCount: number): any[] {
    const swarm = [];
  ...
  private initializeVelocity(problem: OptimizationProblem): any {
    ...
    return { channelWeights: Array(5).fill(0).map(() => (Math.random() - 0.5) * 0.1) };
  }
  ...
  private updateParticleVelocity(particle: any, globalBest: any, params: ParticleSwarmParams): void {
    const r1 = Math.random();
    const r2 = Math.random();

    // Update velocity components
    if (particle.velocity.channelWeights && particle.personalBest.variables.channelWeights) {
      for (let i = 0; i < particle.velocity.channelWeights.length; i++) {
        particle.velocity.channelWeights[i] = 
          ...
          params.cognitiveWeight * r1 * (particle.personalBest.variables.channelWeights[i] - particle.variables.channelWeights[i]) +
          params.socialWeight * r2 * (globalBest.variables.channelWeights[i] - particle.variables.channelWeights[i]);
        
        // Clamp velocity
        particle.velocity.channelWeights[i] = Math.max(-params.velocityClamp, 
  ...
  private updateParticlePosition(particle: any, problem: OptimizationProblem): void {
    if (particle.velocity.channelWeights && particle.variables.channelWeights) {
      for (let i = 0; i < particle.variables.channelWeights.length; i++) {
        particle.variables.channelWeights[i] += particle.velocity.channelWeights[i];
        particle.variables.channelWeights[i] = Math.max(0, Math.min(1, particle.variables.channelWeights[i]));
      }
      
      // Normalize weights
      const sum = particle.variables.channelWeights.reduce((a: number, b: number) => a + b, 0);
      particle.variables.channelWeights = particle.variables.channelWeights.map((w: number) => w / sum);
    }
  }

  // Helper methods for other algorithms
  private generateNeighbor(solution: any, problem: OptimizationProblem, neighborhoodSize: number): any {
  ...
  private generateAllNeighbors(solution: any, problem: OptimizationProblem): any[] {
    const neighbors = [];
  ...
  private createTabuMove(from: any, to: any): any {
    return { from: JSON.stringify(from.variables), to: JSON.stringify(to.variables) };
  }

  private solutionsEqual(solution1: any, solution2: any): boolean {
    return JSON.stringify(solution1.variables) === JSON.stringify(solution2.variables);
  }

  private selectRandomIndices(populationSize: number, exclude: number, count: number): number[] {
    const indices = [];
  ...
  private createMutantVector(a: any, b: any, c: any, F: number, problem: OptimizationProblem): any {
    const mutant = JSON.parse(JSON.stringify(a));
    
    // Apply differential evolution mutation
    if (a.variables.channelWeights && b.variables.channelWeights && c.variables.channelWeights) {
      for (let i = 0; i < a.variables.channelWeights.length; i++) {
        mutant.variables.channelWeights[i] = a.variables.channelWeights[i] + 
          F * (b.variables.channelWeights[i] - c.variables.channelWeights[i]);
        mutant.variables.channelWeights[i] = Math.max(0, Math.min(1, mutant.variables.channelWeights[i]));
      }
      
      // Normalize
      const sum = mutant.variables.channelWeights.reduce((a: number, b: number) => a + b, 0);
      mutant.variables.channelWeights = mutant.variables.channelWeights.map((w: number) => w / sum);
    }
  ...
  private crossoverDE(target: any, mutant: any, CR: number, problem: OptimizationProblem): any {
    ...
    if (target.variables.channelWeights && mutant.variables.channelWeights) {
      for (let i = 0; i < target.variables.channelWeights.length; i++) {
        if (Math.random() < CR) {
          trial.variables.channelWeights[i] = mutant.variables.channelWeights[i];
        }
      }
    }
  ...
  getMetrics(problemId?: string): OptimizationMetrics[] {
    if (problemId) {
      return Array.from(this.metrics.values()).filter(m => m.problemId === problemId);
    }
